...
void main() {
    ...
    vec3 glowRgb = (blurA.rgb * 1.08 + blurB.rgb * 0.72 + blurC.rgb * 0.32) * sunsetBoost * pulse;
    ...
    float glowAlpha = (blurA.a * 0.52 + blurB.a * 0.34 + blurC.a * 0.15) * sunsetBoost * pulse;
    glowAlpha = clamp(glowAlpha * mix(0.72, 1.0, softMask), 0.0, 0.46);
    glowRgb = min(glowRgb, vec3(0.70));
